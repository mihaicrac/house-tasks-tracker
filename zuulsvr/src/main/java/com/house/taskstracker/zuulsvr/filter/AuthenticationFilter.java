...
public class AuthenticationFilter extends ZuulFilter {
    ...
    private UUID isAuthTokenValid() {
        ResponseEntity<UUID> restExchange = null;
        try {
            restExchange = restTemplate.exchange("http://authenticationservice/v1/validate/{token}",
                                                 HttpMethod.GET,
                                                 null,
                                                 UUID.class,
    ...
    public Object run() {
        ...
        UUID userId = isAuthTokenValid();
        if (userId != null) {
            filterUtils.setUserId(userId.toString());
